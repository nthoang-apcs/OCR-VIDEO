#include "MyCannyEdge.h"

const double M_PI = 3.14159265;

MyCannyEdge::MyCannyEdge(String filename)
{
	img = imread(filename);
	...
	else
	{
		...
		grayscaled = Mat(toGrayScale(img)); //Grayscale the image
...
Mat MyCannyEdge::toGrayScale(Mat img)
{
...
vector<vector<double>> MyCannyEdge::createFilter(int row, int column, double sigmaIn)
{
...
Mat MyCannyEdge::useFilter(Mat img_in, vector<vector<double>> filterIn)
{
...
Mat MyCannyEdge::threshold(Mat imgin, int low, int high)
{
	if (low > 255)
		low = 255;
	if (high > 255)
